...
export class HttpService {
  ...
  private request<T>(method: 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE', options: any): Observable<T> {
    ...
    switch (method) {
      case 'GET':
        request = this.httpClient.get<T>(`${this.apiBaseUrl}/${options.url}`, { headers, params: options.params, responseType: options.responseType });
        break;
      case 'POST':
        request = this.httpClient.post<T>(`${this.apiBaseUrl}/${options.url}`, options.body, { headers, responseType: options.responseType });
        break;
      case 'PUT':
        request = this.httpClient.put<T>(`${this.apiBaseUrl}/${options.url}`, options.body, { headers, responseType: options.responseType });
        break;
      case 'PATCH':
        request = this.httpClient.patch<T>(`${this.apiBaseUrl}/${options.url}`, options.body, { headers, responseType: options.responseType });
        break;
      case 'DELETE':
        request = this.httpClient.delete<T>(`${this.apiBaseUrl}/${options.url}`, { headers, params: options.params, responseType: options.responseType });
        break;
      default:
